...
stumpM = 30
start = 60
dataStreamInit = np.zeros(shape = 800, dtype = np.float64)
itterator = 0
...
while True:
    ...
    if ret == True:
        ...
        dataStreamInit[itterator] = p1[1]
        


        if len(data) == 60:
            print(dataStreamInit[:60])
            stream = stumpy.stumpi(dataStreamInit[:60], stumpM)
        

        if len(data) >= 60:
            
            print(dataStreamInit[itterator])
            cv2.putText(img, "adding", [300,400], font, 2, (255, 0, 0), 3)
            stream.update(dataStreamInit[itterator].astype(np.float64))
